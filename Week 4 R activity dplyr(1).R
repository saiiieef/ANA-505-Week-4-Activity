...
data(HairEyeColor)
h_e_color <- data.frame(HairEyeColor)
...
hair_sex <- select(h_e_color,Hair, Sex)

#Let's get rid of the Sex column in the new dataset created above
#TASK: Write the function that deselects the sex column
#(hint: use the 'select' function to not select a -column)
select(h_e_color,Hair, -c(Sex))

#Let's rename a column name
#TASK: Write the function that renames 'sex' to 'gender'
rename(hair_sex,Hair,gender=Sex)
...
male_hair_gender <- filter(hair_gender, gender =="Male")
...
total=mutate(h_e_color, total=cumsum(Freq))


#Since we have a males dataset, let's make a females dataset
#TASK: Write the function that includes only rows that are 'female'
female_hair_gender <- filter(hair_gender, gender =="Female")

#And now let's join the males and females
#TASK: Write the function that joins the male and female rows 
#(hint: try using 'union' or 'bind_rows')
union(female_hair_gender,male_hair_gender)
union_all(male_hair_gender,female_hair_gender)
bind_rows(female_hair_gender,male_hair_gender)

#Optional Task: add any of the other functions 
#you learned about from the dplyr package
transmute(h_e_color, total=cumsum(Freq))